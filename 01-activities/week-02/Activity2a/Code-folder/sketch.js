...
function draw() {
  ...
  if (mouseIsPressed && x > width/2) {
    ellipse(x, height / 2, size+30);
    fill(bgColor);
    strokeWeight(2)
  } 
  else if (mouseIsPressed && x <= width/2)  {
    rect(x, height / 2, size+30, size+30);
    fill(bgColor);
    strokeWeight(2)
  }
  else {
...
function keyPressed() {
  switch (key) {
    case "1":
      bgColor = color(250, 219, 135);
      break; 
    case "2":
      bgColor = color(250, 135, 235);
      break; 
    case "3":
      bgColor = color(135, 242, 250);
      break; 
    default:
